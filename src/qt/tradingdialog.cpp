    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->PENTAQUARKAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(BTC)"<< "PENTAQUARK(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)"  << "PENTAQUARK(SIZE)" << "TOTAL(BTC)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(PENTAQUARK)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //PENTAQUARKst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-LHC");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-LHC&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-LHC&count=100");
      return Response;
}
...
QString tradingDialog::BuyPENTAQUARK(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-LHC&quantity=";
...
QString tradingDialog::SellPENTAQUARK(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-LHC";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=LHC";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-LHC&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>PENTAQUARK Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> PENTAQUARK");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double PENTAQUARKSupply = 0;
    double PENTAQUARKDemand = 0;
    double BtcSupply  = 0;
    double BtcDemand  = 0;
    ...
    foreach (const QJsonValue & value, SellArray)
    {
        ...
        PENTAQUARKSupply = PENTAQUARKSupply + y;
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        PENTAQUARKDemand = PENTAQUARKDemand + y;
        ...
        ui->PENTAQUARKSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(PENTAQUARKSupply,'i',8) + "</span><b> PENTAQUARK</b>");
        ...
        ui->PENTAQUARKDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(PENTAQUARKDemand,'i',8) + "</span><b> PENTAQUARK</b>");
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       QString balance = GetBalance("PENTAQUARK");
                                       QString str;
                                       QJsonObject ResultObject =  GetResultObjectFromJSONObject(balance);

                                       ui->PENTAQUARKAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("BTC");
                         ...
                         Response = GetBalance("PENTAQUARK");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->PENTAQUARKBalanceLabel,*ui->PENTAQUARKAvailableLabel,*ui->PENTAQUARKPendingLabel, QString::fromUtf8("PENTAQUARK"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputPENTAQUARK->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of BTC that can be gained from selling PENTAQUARK available balance
    QString responseA = GetBalance("PENTAQUARK");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailablePENTAQUARK = ResultObject["Available"].toDouble();

    ui->UnitsInputPENTAQUARK->setText(str.number(AvailablePENTAQUARK,'i',8));
}
...
void tradingDialog::on_BuyPENTAQUARK_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyPENTAQUARK(Order,Quantity,Rate);
...
void tradingDialog::on_SellPENTAQUARKBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputPENTAQUARK->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputPENTAQUARK->text();
            Msg += " PENTAQUARK @ ";
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellPENTAQUARK(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputPENTAQUARK_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
